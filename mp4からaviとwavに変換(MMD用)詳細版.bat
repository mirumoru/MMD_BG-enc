...
ffmpeg -i %INPUT_FOLDER%\%%i -r 30 -vcodec rawvideo %OUTPUT_OPTIONS%  -an %OUTPUT_FOLDER%%%x_MMD用.avi
echo %%iの動画処理が完了しました。
...
rem 音声変更処理を実行
echo 音声をデコードしています。
ffmpeg -i %INPUT_FOLDER%\%%i -sample_fmt s16 -ar 48000 %OUTPUT_FOLDER%%%x_MMD用.wav
echo %%iの音声処理が完了しました。
echo ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
rem --------------------------------------------------------------------------------
    )
