...
set OUTPUT_FOLDER=%INPUT_FOLDER%\Ae_out_MMD\
...
ffmpeg -i %INPUT_FOLDER%\%%i -r 30 -b:v 1500K -vcodec rawvideo -an %OUTPUT_FOLDER%%%x_MMD用Ae版.avi
echo %%iの動画処理が完了しました。
...
rem 音声変更処理を実行
echo 音声をデコードしています。
ffmpeg -i %INPUT_FOLDER%\%%i -sample_fmt s16 -ar 48000 %OUTPUT_FOLDER%%%x_MMD用Ae版.wav
echo %%iの音声処理が完了しました。
echo ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
rem --------------------------------------------------------------------------------
    )
