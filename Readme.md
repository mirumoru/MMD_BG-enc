...
##### [GUI版のDownloads(ダウンロード)>>](https://bowlroll.net/file/271835)  

【ご報告】  
現在「ffmpeg_nvenc版」「ffmpeg_QSV版」「NVEnc版」「QSVEncC版」  
でMMDで背景として読み込む事が確認できませんでした。  
引き続き「簡易版」「詳細版」は確認できました。  
原因調査中です。また、私のPCが原因の可能もありますので、  
一度使ってみて、できない場合は上記のbatファイルをご利用ください。  
...
## 《batファイルの種類》  
簡易版・詳細版(CPU)でエンコードした動画は20GBでも読み込む事が出来ます。
(絶対に読み込むかは分からない)  
...
「今回の更新 v1.5.2」2022/03/05  
ffmpegのバージョンを6.0にしました。また、「バージョン確認バッチ」,「NVENC64利用可能確認バッチ」を修正しました。  
【いろいろ調べましたが、原因を特定することができませんでした。すみません。】  
ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー

《今までの更新内容》

「v1.5.1」2022/09/07
QSVEncC版を追加しましたがMMDで動作確認が出来ませんでした。
そのため「現在原因調査中」ファイルの中に入っています。
また「ffmpeg_nvenc版」「ffmpeg_QSV版」「NVEnc版」も移動しています。
